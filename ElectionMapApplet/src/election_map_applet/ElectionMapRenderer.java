...
public class ElectionMapRenderer extends Renderer
{
	 ...
	 * @param name:String
	 */
	public void victory(Candidate winner, Graphics g){
		//String output = winner.getName() + " WON!";
		String output = "John McCain WON!";
		...
		for(int i=0; outputTokens.hasMoreTokens(); i++){
			...
			if(outputTokens.hasMoreTokens()){
				...
				g.drawString(currentOutput, x+(displacement*11), 660);
			}
			else{
				g.setColor(Color.green);
				g.drawString(currentOutput, x+(i*65), 660);
			}
